...
Material colors palette to use in :class:`eze.theming.ThemeManager`.
...
.. code-block:: python
    ...
    class Tab(EZEBoxLayout, EZETabsBase):
        pass


    class ItemColor(EZEBoxLayout):
        text = StringProperty()
        color = ListProperty()


    class Palette(EZEApp):
        ...
        def on_tab_switch(
            ...
            for value_color in colors[tab_text]:
                self.screen.ids.rv.data.append(
                    {
                        "viewclass": "ItemColor",
                        "eze_bg_color": colors[tab_text][value_color],
                        "title": value_color,
                    }
                )
