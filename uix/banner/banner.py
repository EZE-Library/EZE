...
To use a two-line banner, specify the ``'two-line'`` :attr:`EZEBanner.type` for the banner
and pass the list of two lines to the :attr:`EZEBanner.text` parameter:
...
use the :attr:`EZEBanner.left_action` and :attr:`EZEBanner.right_action` parameters,
...
class EZEBanner(EZECard):
    """
    Banner class.

    For more information, see in the :class:`~eze.uix.card.EZECard`
    class documentation.
    """
    ...
    def add_actions_buttons(
        self, instance_box: EZEBoxLayout, data: list
    ) -> None:
        """
