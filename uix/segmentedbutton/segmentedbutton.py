...
:attr:`eze.uix.segmentedbutton.segmentedbutton.EZESegmentedButton.multiselect`
parameter:
...
:class:`~eze.uix.segmentedbutton.segmentedbutton.EZESegmentedButton` and
:class:`~eze.uix.segmentedbutton.segmentedbutton.EZESegmentedButtonItem`
classes are inherited and use all their attributes such as
`eze_bg_color`, `eze_bg_color` etc. for additional customization of segments.
...
.. code-block:: python
    ...
    def on_marked(
        self,
        segment_button: EZESegmentedButton,
        segment_item: EZESegmentedButtonItem,
        marked: bool,
    ) -> None:
...
.. code-block:: python
    ...
    from eze.uix.segmentedbutton import EZESegmentedButton, EZESegmentedButtonItem
...
__all__ = ("EZESegmentedButton", "EZESegmentedButtonItem")
...
from eze.uix.behaviors import RectangularRippleBehavior, ScaleBehavior
from eze.uix.boxlayout import EZEBoxLayout
...
class EZESegmentedButtonItem(
    RectangularRippleBehavior, ButtonBehavior, EZEFloatLayout
):
    """
    Segment button item.
    ...
    def on_disabled(self, instance, value: bool) -> None:
        def on_disabled(*args):
            if value:
                ...
                if self._current_eze_bg_color:
                    self.eze_bg_color = self._current_eze_bg_color
                    self._current_eze_bg_color = None
...
class EZESegmentedButton(EZEBoxLayout):
    """
    ...
    """
    The list of :class:`~EZESegmentedButtonItem` objects that are currently
    marked.
    ...
    def shift_segment_text(self, segment_item: EZESegmentedButtonItem) -> None:
        """
    ...
    def show_icon_marked_segment(
        self, segment_item: EZESegmentedButtonItem
    ) -> None:
        """
    ...
    def restore_bg_segment(self, segment_item) -> None:
        Animation(eze_bg_color=self.eze_bg_color, d=0.2).start(segment_item)

    def set_bg_marked_segment(self, segment_item) -> None:
        if segment_item.active:
            Animation(
                eze_bg_color=self.selected_color
    ...
    def mark_item(self, segment_item: EZESegmentedButtonItem) -> None:
    ...
    def add_widget(self, widget, *args, **kwargs):
        if isinstance(widget, EZESegmentedButtonItem):
...
class SegmentButtonIcon(EZEIcon, ScaleBehavior):
