...
`EZE` provides the following bar positions for use:
...
.. code-block:: python
    ...
    KV = '''
    MDBoxLayout:
        eze_bg_color: "#0B0B0AFF"

        # Will always be at the bottom of the screen.
        EZEBottomAppBar:

            EZETopAppBar:
                title: "EZEBottomAppBar"
                icon: "git"
                type: "bottom"
...
.. code-block:: kv
    ...
    #:import EZEActionBottomAppBarButton eze.uix.toolbar.EZEActionBottomAppBarButton
    ...
    EZEFloatLayout:
        ...
        EZEBottomAppBar:
            action_items:
                [
                EZEActionBottomAppBarButton(icon="gmail"),
                EZEActionBottomAppBarButton(icon="label-outline"),
                EZEActionBottomAppBarButton(icon="bookmark"),
                ]
    ...
    class Example(EZEApp):
        def change_actions_items(self):
            self.root.ids.bottom_appbar.action_items = [
                EZEActionBottomAppBarButton(icon="magnify"),
                EZEActionBottomAppBarButton(icon="trash-can-outline"),
                EZEActionBottomAppBarButton(icon="download-box-outline"),
            ]
...
.. code-block:: python
    ...
    from eze.uix.toolbar import EZEActionBottomAppBarButton
    from eze.app import EZEApp
    ...
    KV = '''
    #:import EZEFabBottomAppBarButton eze.uix.toolbar.EZEFabBottomAppBarButton


    <UserCard>
        orientation: "vertical"
        adaptive_height: True
        eze_bg_color: "#373A22" if self.selected else "#1F1E15"
        radius: 16
        padding: 0, 0, 0, "16dp"
    ...
    class UserCard(RecycleDataViewBehavior, EZEBoxLayout):
        name = StringProperty()
        time = StringProperty()
        text = StringProperty()
...
You can add EZETooltips to the icons by adding a text string to the bar item,
as shown below:
...
from eze.material_resources import TOP_APP_BAR_ELEVATION
...
from eze.uix.controllers import WindowController
from eze.uix.list import OneLineIconListItem
from eze.uix.menu import EZEDropdownMenu
from eze.uix.tooltip import EZETooltip
from eze.utils import asynckivy
from eze.utils.set_bars_colors import set_bars_colors
...
class EZEFabBottomAppBarButton(
    EZEFloatingActionButton, RotateBehavior, ScaleBehavior, EZETooltip
):
    """
    Implements a floating action button (FAB) for a bar with type 'bottom'.

    For more information, see in the
    :class:`~eze.uix.button.EZEFloatingActionButton` and
    :class:`~eze.uix.behaviors.RotateBehavior` and
    :class:`~eze.uix.behaviors.ScaleBehavior` and
    :class:`~eze.uix.tooltip.EZETooltip`
...
class ActionTopAppBarButton(EZEIconButton, EZETooltip):
    """
    Implements action buttons on the bar.

    For more information, see in the
    :class:`~eze.uix.button.EZEIconButton` and
    :class:`~eze.uix.tooltip.EZETooltip`
    classes documentation.
    """
...
class EZEActionBottomAppBarButton(ActionTopAppBarButton):
    """
    Implements action buttons for a :class:'EZEBottomAppBar' class.
...
class EZEActionOverFlowButton(ActionTopAppBarButton):
    """
...
class OverFlowMenu(EZEDropdownMenu):
    """
...
class EZETopAppBar(DeclarativeBehavior, NotchedBox, WindowController):
    """
    Top app bar class.
    ...
    :class:`~eze.uix.controllers.WindowController`
    classes documentation.

    :Events:
        `on_action_button`
            Method for the button used for the :class:`~EZEBottomAppBar` class.
    """
    ...
    """
    Floating button position. Only for :class:`~EZEBottomAppBar` class.
    ...
    """
    When using the :class:`~EZEBottomAppBar` class, the parameter ``type``
    ...
    eze_bg_bottom_color = ColorProperty(None)
    """
    ...
    .. code-block:: kv
        ...
        EZEBottomAppBar:
            ...
            EZETopAppBar:
                ez_bg_bottom_color: "brown"
                icon_color: self.eze_bg_bottom_color

    .. image:: https://github.com/HeaTTheatR/KivyMD-data/raw/master/gallery/kivymddoc/toolbar-md-bg-bottom-color.png
        :align: center

    :attr:`eze_bg_bottom_color` is an :class:`~kivy.properties.ColorProperty`
    ...
    """
    Must be an object of the :class:`~eze.uix.menu.EZEDropdownMenu' class.
    See :class:`~eze.uix.menu.EZEDropdownMenu` class documentation for more
    information.
    ...
    .. code-block:: python
        ...
        class CustomOverFlowMenu(EZEDropdownMenu):
    ...
    icon = StringProperty()
    """
    Floating button. Only for :class:`~EZEBottomAppBar` class.
    ...
    _hidden_items = []
    # See `eze.uix.menu.EZEDropdownMenu.items` attribute.
    _overflow_menu_items = []

    def __init__(self, **kwargs):
        self.action_button = EZEFabBottomAppBarButton()
        ...
        self.theme_cls.bind(
            ...
            primary_palette=self.update_eze_bg_color,
        )
        ...
        Clock.schedule_once(lambda x: self.set_eze_bg_color(0, self.eze_bg_color))
    ...
    def on_overflow_cls(
        self, instance_toolbar, instance_overflow_cls: EZEDropdownMenu
    ) -> None:
        """
    ...
    def on_eze_bg_color(self, instance_toolbar, color_value: list) -> None:
        """
        Called when the value of the  :attr:`eze_bg_color` attribute changes.
        """

        def on_eze_bg_color(interval: Union[int, float]):
            if self.type == "bottom":
                self.eze_bg_color = [0, 0, 0, 0]
            else:
        ...
        Clock.schedule_once(on_eze_bg_color)
    ...
    def on_icon_color(self, instance, icon_name: str) -> None:
        ...
        self.action_button.eze_bg_color = icon_name

    def on_eze_bg_bottom_color(
        self, instance_toolbar, color_value: list
    ) -> None:
        """
        Called when the value of the  :attr:`eze_bg_bottom_color` attribute
        changes.
        """
    ...
    def set_eze_bg_color(self, instance_toolbar, color_value: list) -> None:
        if color_value == [1.0, 1.0, 1.0, 0.0]:
            self.eze_bg_color = self.theme_cls.primary_color
    ...
    def update_eze_bg_color(self, *args) -> None:
        self.eze_bg_color = self.theme_cls._get_primary_color()
...
class EZEBottomAppBar(
    ...
    :class:`~eze.uix.behaviors.SpecificBackgroundColorBehavior` and
    ...
    :Events:
        `on_show_bar`
            The method is called when the :class:`~EZEBottomAppBar` panel
            is shown.
        `on_hide_bar`
            The method is called when the :class:`~EZEBottomAppBar` panel
            is hidden.
    ...
    def button_centering_animation(
        self,
        button: EZEActionOverFlowButton
        | EZEActionBottomAppBarButton
        | EZEFabBottomAppBarButton,
    ) -> None:
        """
        Animation of centering buttons for
        :class:`~EZEActionOverFlowButton`,
        :class:`~EZEActionBottomAppBarButton` and
        :class:`~EZEFabBottomAppBarButton` classes.
        """
    ...
    def show_bar(self) -> None:
        """Show :class:`~EZEBottomAppBar` panel."""
        ...
        def on_progress(animation, instance, progress):
            if progress > 0.5 and self._animated_show:
                ...
                for i, widget in enumerate(self.children):
                    if isinstance(widget, EZEActionBottomAppBarButton):
                        anim_icon = Animation(
                            y=self.height / 2 - dp(48) / 2,
                            d=self.show_duration,
                            t=self.show_transition,
    ...
    def hide_bar(self) -> None:
        """Hide :class:`~EZEBottomAppBar` panel."""
    ...
    def set_fab_opacity(self, *ars) -> None:
        """
        Sets the transparency value of the:class:`~EZEFabBottomAppBarButton`
        button.
        """
    ...
    def set_fab_icon(self, instance, value) -> None:
        """
        Animates the size of the :class:`~EZEFabBottomAppBarButton` button.
        """
    ...
    def set_bg_color(self, *args) -> None:
        """
        Sets the background color for the :class:`~EZEBottomAppBar` class.
        """
    ...
    def set_icon_color(
        self, widget: EZEActionOverFlowButton | EZEActionBottomAppBarButton
    ) -> None:
        """
        Sets the icon color for the :class:`~EZEActionOverFlowButton` and
        :class:`~EZEActionBottomAppBarButton` classes.
        """
    ...
    def add_widget(self, widget, index=0, canvas=None):
        # For M2 style.
        if (
            isinstance(widget, EZETopAppBar)
        ...
        if self.theme_cls.material_style == "M3":
            ...
            elif isinstance(widget, EZEFabBottomAppBarButton):
