...
from eze.uix.hero import EZEHeroFrom, EZEHeroTo
from eze.uix.screenmanager import EZEScreenManager


class EZETransitionBase(TransitionBase):
    """
    TransitionBase is used to animate 2 screens within the
    :class:`~eze.uix.screenmanager.EZEScreenManager`.
    ...
    _direction = "in"
    # Collection of child widgets of all 'EZEHeroFrom' widgets that are
    ...
    #
    # {
    #     'kivy':   <eze.uix.fitimage.fitimage.FitImage object>,
    #     'eze': <eze.uix.fitimage.fitimage.FitImage object>,
    # }
    _hero_from_widget_children = DictProperty()

    def start(self, instance_screen_manager: EZEScreenManager) -> None:
            ...
            for hero_from_widget in self.manager.get_hero_from_widget():
                for heroes_tag in self.manager.current_heroes:
                    if heroes_tag == hero_from_widget.tag:
                        ...
                        # Adding a child widget from the 'EZEHeraFrom' container
                        # to the 'EZEHeroTo' container.
                        ...
                        # Restores the child widget for the 'EZEHeraFrom'
                        # container.
    ...
    def _check_widget_properties(self, hero_from_widget: EZEHeroFrom):
        if not self.screen_in.heroes_to:
            raise Exception(
                f"The `heroes_to` attribute is not specified for screen "
                f"{self.screen_in}"
            )
        # The 'EZEHeroFrom' widget allows you to place only one widget in
        # itself.
    ...
    def _check_hero_to_widget_tag(
        self, hero_to_widget: EZEHeroTo, hero_from_widget: EZEHeroFrom
    ) -> None:
...
class EZESwapTransition(SwapTransition, EZETransitionBase):
    pass


class EZESlideTransition(SlideTransition, EZETransitionBase):
    pass


class EZEFadeSlideTransition(EZESlideTransition):
    def start(self, instance_screen_manager: EZEScreenManager) -> None:
        if self.is_active:
