...
.. rubric:: :class:`~EZEBottomNavigationItem` provides the following events for use:
...
Use method :attr:`~EZEBottomNavigation.switch_tab` which takes as argument
...
from eze.material_resources import STANDARD_INCREMENT
from eze.theming import ThemableBehavior, ThemeManager
from eze.uix.anchorlayout import EZEAnchorLayout
from eze.uix.behaviors import CommonElevationBehavior, DeclarativeBehavior
from eze.uix.behaviors.backgroundcolor_behavior import (
...
class EZEBottomNavigationHeader(ButtonBehavior, EZEAnchorLayout):
    """
    Bottom navigation header class.

    For more information, see in the
    :class:`~kivy.uix.behaviors.ButtonBehavior` and
    :class:`~eze.uix.anchorlayout.EZEAnchorLayout`
    ...
    tab = ObjectProperty()
    """
    :attr:`tab` is an :class:`~EZEBottomNavigationItem`
    and defaults to `None`.
    """

    panel = ObjectProperty()
    """
    :attr:`panel` is an :class:`~EZEBottomNavigation`
    and defaults to `None`.
    """
    ...
    text = StringProperty()
    """
    :attr:`text` is an :class:`~EZETab.text`
    and defaults to `''`.
    """
...
class EZETab(EZEScreen):
    ...
    def __repr__(self):
        return f"<EZETab name='{self.name}', text='{self.text}'>"


class EZEBottomNavigationItem(EZETab):
    header = ObjectProperty()
    """
    :attr:`header` is an :class:`~EZEBottomNavigationHeader`
    and defaults to `None`.
    """
...
class TabbedPanelBase(
    ...
    For more information, see in the :class:`~eze.theming.ThemableBehavior`
    and :class:`~eze.uix.behaviors.SpecificBackgroundColorBehavior`
    ...
    """
    :attr:`previous_tab` is an :class:`~EZETab` and defaults to `None`.
    """
...
class EZEBottomNavigation(DeclarativeBehavior, TabbedPanelBase):
    """
    ...
    """
    Use text for :class:`~EZEBottomNavigationItem` or not.
    If ``True``, the :class:`~EZEBottomNavigation` panel height will be reduced
    by the text height.
    ...
    """
    :attr:`first_widget` is an :class:`~EZEBottomNavigationItem`
    and defaults to `None`.
    """

    tab_header = ObjectProperty()
    """
    :attr:`tab_header` is an :class:`~EZEBottomNavigationHeader`
    and defaults to `None`.
    """
    ...
    def refresh_tabs(self, *args) -> None:
        ...
        if self.ids:
            ...
            for tab in tab_manager.screens:
                self.tab_header = EZEBottomNavigationHeader(tab=tab, panel=self)
    ...
    def on_text_color_normal(
        ...
        EZEBottomNavigationHeader.text_color_normal = color
        for tab in self.ids.tab_bar.children:
            if not tab.active:
    ...
    def on_text_color_active(
        ...
        def on_text_color_active(*args):
            EZEBottomNavigationHeader.text_color_active = color
    ...
    def add_widget(self, widget, **kwargs):
        if isinstance(widget, EZEBottomNavigationItem):
            self.widget_index += 1
    ...
    def _get_switchig_tab(self, name_tab: str) -> EZEBottomNavigationItem:
...
class EZEBottomNavigationBar(CommonElevationBehavior, EZEFloatLayout):
    pass
