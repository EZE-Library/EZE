...
from eze.tools.patterns.create_project import (
...
def main():
    ...
    #  'from eze.tools.hotreload.app import EZEApp' in the project can just
    ...
    with open(os.path.join(path_to_project, "main.py")) as main_module:
        if "from eze.tools.hotreload.app import EZEApp" in main_module.read():
            use_hotreload = "yes"
        else:
            use_hotreload = "no"
        create_controller(
    ...
    Logger.info(
        f"EZE: The {name_view} view has been added to the project..."
    )
