...
Use a clean architecture for your applications. EZE allows you to quickly
...
Kivy version 2.1.0 and EZE master version.
...
Kivy version 2.1.0, EZE master version and a wrapper for working with
...
.. warning:: On Windows, hot reloading of Python files may not work.
    ...
    please report it to the EZE community.
"""
...
from eze import path as eze_path
from eze.tools.argument_parser import ArgumentParserWithHelp
...
temp_code_responsive_view = '''from eze.uix.responsivelayout import MDResponsiveLayout
...
temp_responsive_platform_baseclass = """from eze.uix.screen import MDScreen
...
from eze.uix.screen import EZScreen

from Utility.observer import Observer


class BaseScreenView(EZEScreen, Observer):
    """
    ...
    """
    Screen manager object - :class:`~eze.uix.screenmanager.EZEScreenManager`.
    ...
    def __init__(self, **kw):
        ...
        self.app = EZEApp.get_running_app()
...
from eze.tools.hotreload.app import EZEApp
...
class {}(EZEApp):
    ...
    def build_app(self) -> EZEScreenManager:
        """
        ...
        self.manager_screens = EZEScreenManager(){}{}
...
class {}(EZEApp):{}
    def __init__(self, **kwargs):
        ...
        self.manager_screens = EZEScreenManager()
        {}
    def build(self) -> EZEScreenManager:
...
def main():
    ...
    if not os.path.exists(
        os.path.join(eze_path, "tools", "patterns", pattern_name)
    ):
        parser.error(
    ...
    if not os.path.exists(path_to_project):
        ...
        Logger.info(f"EZE: Project '{path_to_project}' created")
        Logger.info(
            f"EZE: Create a virtual environment for '{path_to_project}' project..."
        )
        create_virtual_environment()
        Logger.info(
            f"EZE: Install requirements for '{path_to_project}' project..."
        )
...
def install_requirements() -> None:
    ...
    os.system(
        f"{python} -m pip install https://github.com/eze/EZE/archive/master.zip"
    )
