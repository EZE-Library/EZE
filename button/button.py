...
`EZE` provides the following button classes for use:

- EZEIconButton_
- EZEFloatingActionButton_
- EZEFlatButton_
- EZERaisedButton_
- EZERectangleFlatButton_
- EZERectangleFlatIconButton_
- EZERoundFlatButton_
- EZERoundFlatIconButton_
- EZEFillRoundFlatButton_
- EZEFillRoundFlatIconButton_
- EZETextButton_
- EZEFloatingActionButtonSpeedDial_
...
The :class:`~EZEIconButton.icon` parameter must have the name of the icon
from ``eze/icon_definitions.py`` file.
...
.. code-block:: kv
    ...
    EZEIconButton:
        icon: "eze/images/logo/eze-icon-256.png"


By default, :class:`~EZEIconButton` button has a size ``(dp(48), dp (48))``.
...
By default, the color of :class:`~EZEIconButton`
(depending on the style of the application) is black or white.
You can change the color of :class:`~EZEIconButton` as the text color
of :class:`~eze.uix.label.EZELabel`, substituting ``theme_icon_color`` for
...
.. EZEFloatingActionButton:
EZEFloatingActionButton
----------------------


The above parameters for :class:`~EZEIconButton` apply
to :class:`~EZEFloatingActionButton`.

To change :class:`~EZEFloatingActionButton` background, use the
``eze_bg_color`` parameter:
...
.. code-block:: python
    ...
    from eze.app import EZEApp
    from eze.uix.button import EZEFloatingActionButton
    ...
    class Example(EZEApp):
        ...
        def on_start(self):
            data = {
                "standard": {"eze_bg_color": "#fefbff", "text_color": "#6851a5"},
                "small": {"eze_bg_color": "#e9dff7", "text_color": "#211c29"},
                "large": {"eze_bg_color": "#f8d7e3", "text_color": "#311021"},
            }
            for type_button in data.keys():
                self.root.ids.box.add_widget(
                    EZEFloatingActionButton(
                        ...
                        type=type_button,
                        theme_icon_color="Custom",
                        eze_bg_color=data[type_button]["md_bg_color"],
...
To change the text color of: class:`~EZEFlatButton` use the ``text_color`` parameter:
...
.. code-block:: kv
    ...
    EZEFlatButton:
        text: "[color=#00ffcc]EZEFlatButton[/color]"
...
This button is similar to the :class:`~EZEFlatButton` button except that you
can set the background color for :class:`~EZERaisedButton`:

.. code-block:: kv

    EZERaisedButton:
        text: "EZERaisedButton"
        eze_bg_color: "red"
...
Button parameters :class:`~EZERectangleFlatIconButton` are the same as
button :class:`~EZERectangleFlatButton`, with the addition of the
``theme_icon_color`` and ``icon_color`` parameters as for :class:`~EZEIconButton`.

.. code-block:: kv

    EZERectangleFlatIconButton:
        icon: "android"
        text: "EZERectangleFlatIconButton"
        theme_text_color: "Custom"
        text_color: "white"
        line_color: "red"
        theme_icon_color: "Custom"
...
.. code-block:: python
    ...
    from eze.app import EZEApp
    from eze.uix.screen import EZEScreen
    from eze.uix.button import EZERectangleFlatIconButton
...
Button parameters :class:`~EZERoundFlatIconButton` are the same as
button :class:`~EZERoundFlatButton`, with the addition of the
``theme_icon_color`` and ``icon_color`` parameters as for :class:`~EZEIconButton`:

.. code-block:: kv

    EZERoundFlatIconButton:
        text: "EZERoundFlatIconButton"
        icon: "android"
        text_color: "white"
...
Button parameters :class:`~EZEFillRoundFlatButton` are the same as
button :class:`~EZERaisedButton`.
...
Button parameters :class:`~EZEFillRoundFlatIconButton` are the same as
button :class:`~EZERaisedButton`, with the addition of the
``theme_icon_color`` and ``icon_color`` parameters as for :class:`~EZEIconButton`.

.. note:: Notice that the width of the :class:`~EZEFillRoundFlatIconButton`
...
.. EZEloatingActionButtonSpeedDial:
EZEFloatingActionButtonSpeedDial
-------------------------------

.. Note:: See the full list of arguments in the class
    :class:`~EZEFloatingActionButtonSpeedDial`.
...
.. tabs::
    ...
    .. tab:: Imperative python style
        ...
        .. code-block:: python
            ...
            from eze.uix.screen import EZEScreen
            from eze.app import EZEApp
            from eze.uix.button import EZEFloatingActionButtonSpeedDial
            ...
            class Example(EZEApp):
                ...
                def build(self):
                    ...
                    self.theme_cls.primary_palette = "Orange"
                    screen = EZEScreen()
                    speed_dial = EZEFloatingActionButtonSpeedDial()
...
from eze.color_definitions import text_colors
...
from eze.uix.tooltip import EZETooltip
...
class BaseButton(
    ...
    :attr:`eze_bg_color` is a :class:`~kivy.properties.ColorProperty`
    and defaults to `None`.
    """

    eze_bg_color_disabled = ColorProperty(None)
    """
    The background color in (r, g, b, a) or string format of the button when
    the button is disabled.

    :attr:`eze_bg_color_disabled` is a :class:`~kivy.properties.ColorProperty`
    and defaults to `None`.
    """
    ...
    _eze_bg_color = ColorProperty([0.0, 0.0, 0.0, 0.0])
    _eze_bg_color_disabled = ColorProperty([0.0, 0.0, 0.0, 0.0])
    ...
    _default_eze_bg_color = [0.0, 0.0, 0.0, 0.0]
    _default_eze_bg_color_disabled = [0.0, 0.0, 0.0, 0.0]
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.bind(
            eze_bg_color=self.set_button_colors,
            eze_bg_color_disabled=self.set_button_colors,
    ...
    def set_button_colors(self, *args) -> None:
        ...
        _eze_bg_color = (
            self.eze_bg_color
            or self._default_eze_bg_color
            or self.theme_cls.primary_color
        )

        # Set disabled color
        _eze_bg_color_disabled = (
            self.eze_bg_color_disabled
            or (
                [sum(self.eze_bg_color[0:3]) / 3.0] * 3
                + [0.38 if self.theme_cls.theme_style == "Light" else 0.5]
                if self.eze_bg_color
                else None
            )
            or self._default_eze_bg_color_disabled
        ...
        if self.theme_cls.theme_style_switch_animation:
            Animation(
                _eze_bg_color=_eze_bg_color,
                _eze_bg_color_disabled=_eze_bg_color_disabled,
            ...
            self._eze_bg_color = _eze_bg_color
            self._eze_bg_color_disabled = _eze_bg_color_disabled
    ...
    def on_touch_down(self, touch):
        ...
        if touch.is_mouse_scrolling:
            ...
            if self._eze_bg_color[3] == 0.0:
                self._animation_fade_bg = Animation(
                    duration=0.5, _eze_bg_color=[0.0, 0.0, 0.0, 0.1]
                )
    ...
    def on_touch_up(self, touch):
        ...
        if not self.disabled and self._animation_fade_bg:
            self._animation_fade_bg.stop_property(self, "_eze_bg_color")
            ...
            Animation(duration=0.05, _eze_bg_color=eze_bg_color).start(self)
...
class ButtonContentsIcon:
    """
    Contents for a round BaseButton consisting of an :class:`~EZEIcon` class.
    """
...
class EZEFlatButton(BaseButton, ButtonContentsText):
    """
...
class EZERaisedButton(BaseButton, ButtonElevationBehaviour, ButtonContentsText):
    """
    ...
    _default_eze_bg_color = None
    _default_eze_bg_color_disabled = None
...
class EZERectangleFlatButton(BaseButton, ButtonContentsText):
    """
...
class EZERectangleFlatIconButton(
...
class EZERoundFlatButton(BaseButton, ButtonContentsText):
    """
...
class EZEFillRoundFlatButton(BaseButton, ButtonContentsText):
    """
...
class EZEFillRoundFlatIconButton(
    BaseButton, OldButtonIconMixin, ButtonContentsIconText
):
    """
    A flat button with (by default) rounded corners, a primary color fill,
...
class EZEIconButton(BaseButton, OldButtonIconMixin, ButtonContentsIcon):
    """
...
class EZEFloatingActionButton(
    ...
    def on_type(self, instance_eze_floating_action_button, type: str) -> None:
        self.set_size()
        self.set_font_size()


class EZETextButton(ButtonBehavior, EZELabel):
    """
    Text button class.

    For more information, see in the
    :class:`~kivy.uix.behaviors.ButtonBehavior` and
    :class:`~eze.uix.label.EZELabel` classes documentation.
    """
...
class BaseFloatingBottomButton(EZEFloatingActionButton, EZETooltip):
...
class EZEFloatingBottomButton(BaseFloatingBottomButton):
    eze_bg_color = ColorProperty(None)


class EZEFloatingRootButton(RotateBehavior, EZEFloatingActionButton):
    rotate_value_angle = NumericProperty(0)


class EZEFloatingLabel(EZELabel):
...
class EZEFloatingActionButtonSpeedDial(
    ...
    def on_leave(self, instance_button: EZEFloatingBottomButton) -> None:
        """Called when the mouse cursor goes outside the button of stack."""

        if self.state == "open":
            for widget in self.children:
                if isinstance(widget, EZEFloatingLabel) and self.hint_animation:
    ...
    def on_enter(self, instance_button: EZEFloatingBottomButton) -> None:
    ...
    def on_data(self, instance_speed_dial, data: dict) -> None:
        ...
        def on_data(*args):
            ...
            for name, parameters in data.items():
                ...
                bottom_button = EZEFloatingBottomButton(
                    icon=name_icon,
                ...
                floating_text = name
                if floating_text:
                    label = EZEFloatingLabel(text=floating_text, opacity=0)
            ...
            # Top root button.
            root_button = EZEFloatingRootButton(on_release=self.open_stack)
    ...
    def on_icon(self, instance_speed_dial, name_icon: str) -> None:
        self._set_button_property(EZEFloatingRootButton, "icon", name_icon)

    def on_label_text_color(
        self, instance_speed_dial, color: list | str
    ) -> None:
        for widget in self.children:
            if isinstance(widget, EZEFloatingLabel):
                widget.text_color = color

    def on_color_icon_stack_button(
        self, instance_speed_dial, color: list
    ) -> None:
        self._set_button_property(EZEFloatingBottomButton, "icon_color", color)
    ...
    def on_bg_hint_color(self, instance_speed_dial, color: list) -> None:
        setattr(EZEFloatingBottomButton, "_bg_color", color)

    def on_color_icon_root_button(
        self, instance_speed_dial, color: list
    ) -> None:
        self._set_button_property(EZEFloatingRootButton, "icon_color", color)

    def on_bg_color_stack_button(
        self, instance_speed_dial, color: list
    ) -> None:
        self._set_button_property(EZEFloatingBottomButton, "eze_bg_color", color)

    def on_bg_color_root_button(self, instance_speed_dial, color: list) -> None:
        self._set_button_property(EZEFloatingRootButton, "eze_bg_color", color)
    ...
    def set_pos_labels(self, instance_floating_label: EZEFloatingLabel) -> None:
        """
    ...
    def set_pos_root_button(
        self, instance_floating_root_button: EZEFloatingRootButton
    ) -> None:
        """
    ...
    def set_pos_bottom_buttons(
        self, instance_floating_bottom_button: EZEFloatingBottomButton
    ) -> None:
        """
    ...
    def open_stack(
        ...
        if self.state != "open":
            ...
            for widget in self.children:
                if isinstance(widget, EZEFloatingBottomButton):
                ...
                elif isinstance(widget, EZEFloatingLabel):
                    ...
                    isinstance(widget, EZEFloatingRootButton)
    ...
    def do_animation_open_stack(self, anim_data: dict) -> None:
        ...
        :param anim_data:
            {
                <eze.uix.button.EZEFloatingBottomButton object>:
    ...
    def _update_pos_buttons(self, instance, width, height):
        ...
        for widget in self.children:
            ...
            elif isinstance(widget, EZEFloatingRootButton):
